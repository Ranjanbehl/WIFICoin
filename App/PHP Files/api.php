<?php
	...
	function load($requestbody, $responsebody, $sqlconn) {
		...
		if ($row == null) {
			...
			$query = "SELECT SSID, Cost,Lat,Lon,Alt,Password, SQRT(POWER( 6371000 * acos( cos( radians(Lat) ) * cos(  radians($lat)   ) * cos(  radians(Lon) - radians($lon) ) + sin( radians($lat) )* sin( radians( Lat ) )), 2) + POWER($alt - Alt, 2)) AS DISTANCE FROM `Wifi_Data` HAVING DISTANCE < $radius ORDER BY DISTANCE";
			$query_response  = $sqlconn -> query($query);
			$row = $query_response->fetch_assoc();
			while ($row != null) {
				$wifiobj = array("SERIAL" => intval($row["Serial"]), "SSID"=> $row["SSID"], "COST" => intval($row["Cost"]), "LAT"=>$row["Lat"], "LON"=>$row["Lon"], "ALT"=>$row["Alt"], "PASSWORD"=>$row["Password"]);
